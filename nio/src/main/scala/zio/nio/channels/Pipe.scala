...
import zio.{IO, Scope, URIO, ZTraceElement}
...
final class Pipe private (private val pipe: JPipe)(implicit trace: ZTraceElement) {
  ...
  def source(implicit trace: ZTraceElement): URIO[Scope, Pipe.SourceChannel] =
    IO.succeed(new channels.Pipe.SourceChannel(pipe.source())).toNioManaged

  def sink(implicit trace: ZTraceElement): URIO[Scope, Pipe.SinkChannel] =
