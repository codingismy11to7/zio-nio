  ...
  override def spec: Spec[
    ...
    suite("FileChannelSpec")(
      ...
      test("memory mapped buffer") {
        for {
          result <- useNioBlockingOps(FileChannel.open(readFile, StandardOpenOption.READ)) { ops =>
                      for {
      ...
      test("end of stream") {
        useNioBlocking(FileChannel.open(readFile, StandardOpenOption.READ)) { (channel, ops) =>
          for {
            size <- channel.size
      ...
      test("stream reading") {
        useNioBlockingOps(FileChannel.open(readFile, StandardOpenOption.READ)) {
      ...
      test("sink writing") {
        ...
        useNioBlockingOps(FileChannel.open(file, StandardOpenOption.WRITE, StandardOpenOption.CREATE_NEW)) { channel =>
          stream.run(channel.sink())
        }
